...
export const TimelineSection: React.FC<TimelineSectionProps> = () => {
  ...
  // Journey-path scroll reveal
  useGSAP(
    () => {
      const mm = whenMotionOk(() => {
        scrollReveal('[data-tl="header"] > *', { y: 20 }, {
        ...
        scrollReveal('[data-tl="year"]', { x: -20, scale: 0.95 }, {
        ...
        scrollReveal(cardRef.current, { y: 30 }, {
  ...
  return (
    <section
      ref={sectionRef}
      id="timeline"
      className="py-16 sm:py-24 bg-slate-50/70 relative overflow-hidden"
    >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div data-tl="header">
          <SectionHeader
            badge="The 10-Year Creative Odyssey"
            badgeIcon={<Calendar className="w-3.5 h-3.5" />}
            title="A Decade of Imagining, Crafting & Sharing Smiles"
            subtitle="From solitary pencil sketches on a kitchen table in 2014 to a beloved international children's universe with millions of players."
          />
        </div>
        ...
        <div ref={cardRef}>
          <Card elevation="lg" border padded={false} className="overflow-hidden max-w-5xl mx-auto">
            <div className="grid grid-cols-1 lg:grid-cols-12">
              <div className="lg:col-span-5 relative bg-slate-900 min-h-[240px] sm:min-h-[320px] lg:min-h-full lg:self-stretch overflow-hidden">
                <img
                ...
                <div className="absolute top-3 left-3 sm:top-4 sm:left-4 z-10 px-3 py-1 rounded-xl bg-white/95 backdrop-blur-md shadow-md text-slate-900 font-extrabold text-xs sm:text-sm flex items-center gap-1.5">
                  <Star className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-amber-500 fill-amber-500" />
                  <span>{currentMilestone.year} Milestone</span>
                </div>

                <div className="absolute bottom-3 left-3 right-3 sm:bottom-4 sm:left-4 sm:right-4 z-10 p-3 sm:p-3.5 rounded-2xl bg-black/70 backdrop-blur-md border border-white/20 text-white">
                  <p className="text-[10px] sm:text-[11px] font-bold text-pink-300 uppercase tracking-wider">
                    Milestone Achievement
                  </p>
                  <p className="text-xs sm:text-sm font-semibold text-white/95 mt-0.5 line-clamp-2">
                    {currentMilestone.achievement}
                  </p>
                </div>
              </div>

              <div className="lg:col-span-7 p-6 sm:p-8 md:p-10 flex flex-col justify-between space-y-5 sm:space-y-6">
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <span
                      className={`w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full ${currentMilestone.badgeColor}`}
                    />
